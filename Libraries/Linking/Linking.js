 ...
 *     }
 *   }).fail(err => console.error('An error occurred', err));
 * }
 * ```
 *
 ...
 *
 * ```
 * Linking.openURL(url).fail(err => console.error('An error occurred', err));
 * ```
 *
...
class Linking {
  ...
  static getInitialURL(): Promise<?string> {
    if (Platform.OS === 'android') {
      ...
      return Promise(LinkingManagerIOS.initialURL);
    }
  }
